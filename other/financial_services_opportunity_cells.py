...
# ===========================================================================
# CELL 5: VISUALIZATION - Category Overview
# ===========================================================================
"""
## Financial Services Opportunity - Category Comparison
"""

fig, axes = plt.subplots(2, 2, figsize=(16, 12))

# 1. Accounts by Category
...
axes[0, 0].barh(list(category_counts.keys()), list(category_counts.values()), color='#4ECDC4')
axes[0, 0].set_xlabel('Number of Accounts', fontsize=11)
axes[0, 0].set_title('Accounts by Financial Service Category', fontsize=13, fontweight='bold')
axes[0, 0].grid(axis='x', alpha=0.3)

# Add value labels
for i, (cat, val) in enumerate(category_counts.items()):
    axes[0, 0].text(val, i, f' {val:,}', va='center', fontsize=10)
...
axes[0, 1].barh(list(category_spend.keys()), list(category_spend.values()), color='#FF6B6B')
axes[0, 1].set_xlabel('Total Spend ($)', fontsize=11)
axes[0, 1].set_title('Total Spend by Financial Service Category', fontsize=13, fontweight='bold')
axes[0, 1].grid(axis='x', alpha=0.3)

# Add value labels
for i, (cat, val) in enumerate(category_spend.items()):
    axes[0, 1].text(val, i, f' ${val:,.0f}', va='center', fontsize=9)

# 3. Multi-Product Distribution
multi_dist = multi_product_accounts['Category_Count'].value_counts().sort_index()
axes[1, 0].bar(multi_dist.index, multi_dist.values, color='#FFA500', edgecolor='black')
axes[1, 0].set_xlabel('Number of Categories Used', fontsize=11)
axes[1, 0].set_ylabel('Number of Accounts', fontsize=11)
axes[1, 0].set_title('Multi-Product Usage Distribution', fontsize=13, fontweight='bold')
axes[1, 0].grid(axis='y', alpha=0.3)

# Add value labels
for i, (cat_count, acc_count) in enumerate(zip(multi_dist.index, multi_dist.values)):
    axes[1, 0].text(cat_count, acc_count, f'{acc_count:,}', ha='center', va='bottom', fontsize=10)

# 4. Top 10 Categories by Average Spend
avg_spend = {}
for cat, accounts in all_financial_accounts.items():
    avg_spend[cat] = accounts['Total Spend'].mean()

avg_spend = dict(sorted(avg_spend.items(), key=lambda x: x[1], reverse=True)[:10])

axes[1, 1].barh(list(avg_spend.keys()), list(avg_spend.values()), color='#9B59B6')
axes[1, 1].set_xlabel('Average Spend per Account ($)', fontsize=11)
axes[1, 1].set_title('Top 10 Categories by Avg Spend per Account', fontsize=13, fontweight='bold')
...
# ===========================================================================
# CELL 6: EXPORT FINANCIAL SERVICES OPPORTUNITIES
# ===========================================================================
"""
